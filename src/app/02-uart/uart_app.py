...
class Hdlc(threading.Thread):
    ...
    def run(self):
        ...
        while self.connected==True:
            try:
                ...
                if   self._expectedByte == self._EXPECTED_FLAG:
                    if byte==self._HDLC_FLAG:
                        self.receivedFrame            = {}
                        self.receivedFrame['payload'] = []
                        self._expectedByte            = self._EXPECTED_PAYLOAD
                
                elif self._expectedByte==self._EXPECTED_PAYLOAD:
                    
                    if byte==self._HDLC_FLAG:
                        # end of packet
                        # split payload and fcs
                        if len(self.receivedFrame['payload'])>2:
                            ...
                            print "@Hdlc: received hdlc frame too short"
                        self._restart()
                        ...
                        if byte==self._HDLC_ESCAPE:
                            ...
                            if self._escape == True:
                                self.receivedFrame['payload'].append(byte^self._HDLC_MASK)
                                self._escape = False
                            else:
                                self.receivedFrame['payload'].append(byte)
    ...
    def _restart(self):
        self._expectedByte   = self._EXPECTED_FLAG
        self._escape         = False
...
def rxCallback(uartFrame):
    output  = []
    output += ['uart RX ({0} bytes) '.format(len(uartFrame))]
    output += [' '.join('%02x'%b for b in uartFrame[:100])]
    if len(uartFrame)>100:
        output += ['...']
    output  = ''.join(output)
    print output
