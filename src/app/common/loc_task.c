...
#define DFLT_JOINDUTYCYCLE   0xFF
...
typedef struct {
   // configuration
   dnm_cli_cont_t*      cliContext;              ///< Current CLI context
...
*/
void loc_task_init(dnm_cli_cont_t* cliContext,
                   INT8U           fJoin,
                   dn_netid_t      netId,
                   INT16U          udpPort,
                   OS_EVENT*       joinedSem,
   ...
   //===== store params
   notif_task_v.cliContext        = cliContext;
...
Only call this function when the mote has passed the LOC_STATE_CONFIG state.
   ...
   while (1) { // this is a task, it executes forever
      ...
      if (eventFlags & DN_API_LOC_EV_BOOT) {
         ...
         if (notif_task_v.fConfigured==0x00) {
            ...
            // configure the mote's join duty cycle
            if (notif_task_v.fJoin) {
                joinDutyCycle = DFLT_JOINDUTYCYCLE;
                dnErr = dnm_loc_setParameterCmd(
                   DN_API_PARAM_JOINDUTYCYCLE,        // paramId
                   &joinDutyCycle,                    // payload
                   1,                                 // payload length
                   &rc                                // return code
                );
                ASSERT(dnErr==DN_ERR_NONE);
